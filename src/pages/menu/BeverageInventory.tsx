...
const beverageItemSchema = z.object({
  ...
  type: z.enum(["Cocktail", "Wine", "Beer", "Spirit", "Mixer", "Other"], {
...
type BeverageFormData = z.infer<typeof beverageItemSchema>; // Infer type directly from the schema
  ...
  return (
    <div className="min-h-full flex flex-col items-center bg-background text-foreground p-6">
      ...
      <div className="w-full max-w-4xl space-y-8">
            ...
            <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
              ...
              <DialogContent className="sm:max-w-[425px]">
                <DialogHeader>
                  ...
                  <DialogDescription>
                    {editingItem ? "Make changes to the beverage item here. Click save when you're done." : "Add a new beverage to your inventory. Click save when you're done."}
                  </DialogDescription>
                </DialogHeader>
                <Form {...form}>
                  <form onSubmit={form.handleSubmit(onSubmit)} className="grid gap-4 py-4">
                    ...
                    />
                    <DialogFooter>
                      <Button type="submit">{editingItem ? "Save changes" : "Add Beverage"}</Button>
                    </DialogFooter>
                  </form>
                </Form>
              </DialogContent>
        ...
        {/* Display Existing Beverage Inventory */}
        <Card className="bg-card p-6 rounded-lg shadow-md">
          <CardHeader>
            <CardTitle className="text-2xl font-semibold text-primary">Current Beverage Inventory</CardTitle>
            <CardDescription className="text-muted-foreground">A list of all beverages in your inventory.</CardDescription>
          </CardHeader>
          <CardContent>
            {beverageInventory.length === 0 ? (
              <p className="text-muted-foreground text-center">No beverages added yet. Click "Add New Beverage" to get started!</p>
            ) : (
