    ...
    <div className="p-4 bg-white text-gray-900 max-w-4xl mx-auto shadow-lg rounded-lg print:shadow-none print:p-0"> {/* Reduced p-6 to p-4 */}
            ...
            <tbody>
              {proposal.items.map((item) => (
                <tr key={item.id} className="bg-white border-b dark:bg-gray-800 dark:border-gray-700">
                  ...
                  <td className="px-3 py-1 text-right">{currencySymbol}{item.unitCost.toFixed(2)}</td> {/* Reduced px-4 py-2 to px-3 py-1 */}
                  <td className="px-3 py-1 text-right">{currencySymbol}{item.totalCost.toFixed(2)}</td> {/* Reduced px-4 py-2 to px-3 py-1 */}
                </tr>
              ))}
            </tbody>
          </table>
      ...
      <div className="mb-3 flex justify-end"> {/* Reduced mb-4 to mb-3 */}
        <div className="w-full md:w-1/2 space-y-1 text-right">
          <p className="flex justify-between"><span>Labor Cost:</span> <span className="font-semibold">{currencySymbol}{proposal.laborCost.toFixed(2)}</span></p>
          <p className="flex justify-between"><span>Equipment Cost:</span> <span className="font-semibold">{currencySymbol}{proposal.equipmentCost.toFixed(2)}</span></p>
          <p className="flex justify-between"><span>Other Costs:</span> <span className="font-semibold">{currencySymbol}{proposal.otherCosts.toFixed(2)}</span></p>
          <div className="border-t pt-1 mt-1"> {/* Reduced pt-2 mt-2 to pt-1 mt-1 */}
            <p className="flex justify-between text-lg font-bold"><span>Subtotal:</span> <span>{currencySymbol}{proposal.subtotal.toFixed(2)}</span></p>
            <p className="flex justify-between"><span>Tax ({ (proposal.taxRate * 100).toFixed(1) }%):</span> <span>{currencySymbol}{(proposal.subtotal * proposal.taxRate).toFixed(2)}</span></p>
            <p className="flex justify-between text-xl font-extrabold text-primary mt-1"><span>TOTAL:</span> <span>{currencySymbol}{proposal.totalAmount.toFixed(2)}</span></p> {/* Reduced mt-2 to mt-1 */}
          </div>
        </div>
      </div>
